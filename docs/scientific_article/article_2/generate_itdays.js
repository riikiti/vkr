...
function generate() {
  ...
  children.push(bodyParagraph("Коэффициент корреляции r(X, Y) для всех алгоритмов близок к нулю. Наименьшее значение — у ChaCha20 (0,0008), наибольшее — у DES (0,0089)."));

  children.push(bodyParagraph("Лавинный эффект: AES-128 и ChaCha20 демонстрируют минимальное отклонение от идеала 50 %, тогда как DES показывает наибольшее отклонение (49,61 %)."));
  ...
  children.push(bodyParagraph("ChaCha20 и AES-128 занимают лидирующие позиции с интегральной оценкой выше 0,999. Blowfish демонстрирует результаты на уровне AES-128 благодаря эффективной архитектуре [7]. ГОСТ 28147-89 на четвёртом месте. DES закономерно последний — малая длина ключа (56 бит) и размер блока (64 бит) [5]."));
  ...
  children.push(bodyParagraph("Проведено экспериментальное исследование криптостойкости шести симметричных алгоритмов шифрования. Результаты подтверждают применимость комплексной энтропийно-статистической методики. ChaCha20 и AES-128 демонстрируют наивысшую криптостойкость. Устаревший DES показывает статистически значимые отклонения [5]."));
