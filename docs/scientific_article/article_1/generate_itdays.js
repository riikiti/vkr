...
function generate() {
  ...
  // ==================== ВВЕДЕНИЕ ====================
  children.push(p("Введение", { bold: true, align: AlignmentType.CENTER, noIndent: true, before: 120, after: 120 }));
  ...
  children.push(p("Существующие подходы к оценке криптостойкости", { bold: true, align: AlignmentType.CENTER, noIndent: true, before: 120, after: 120 }));
  ...
  // ==================== ЭНТРОПИЙНЫЕ МЕТРИКИ ====================
  children.push(p("Энтропийные метрики оценки криптографических преобразований", { bold: true, align: AlignmentType.CENTER, noIndent: true, before: 120, after: 120 }));
  ...
  // ==================== СИСТЕМА КРИТЕРИЕВ ====================
  children.push(p("Система критериев оценки качества криптографических преобразований", { bold: true, align: AlignmentType.CENTER, noIndent: true, before: 120, after: 120 }));
  ...
  // ==================== ЗАКЛЮЧЕНИЕ ====================
  children.push(p("Заключение", { bold: true, align: AlignmentType.CENTER, noIndent: true, before: 120, after: 120 }));
  ...
  children.push(p("Список литературы", { bold: true, align: AlignmentType.CENTER, noIndent: true, before: 240, after: 120 }));
  ...
  Packer.toBuffer(doc).then(buffer => {
    const outPath = path.join(__dirname, "Курский_статья_1_ITDays.docx");
    ...
    if (fs.existsSync(outPath)) {
      ...
      fs.copyFileSync(outPath, path.join(versionsDir, `Курский_статья_1_ITDays_${ts}.docx`));
    }

    fs.writeFileSync(outPath, buffer);
    console.log("Article 1 (IT'Days) generated:", outPath);
    ...
    const remotePath = "gdrive:ВКР/Статья 1 — IT'Days-2026 — Курский Р.И.docx";
    try {
