...
function generateChapter1() {
  ...
  children.push(normalParagraph([
    ...
    { text: " \u2013 наука о методах обеспечения конфиденциальности, целостности и аутентичности информации. В основе криптографии лежит процесс " },
  ...
  children.push(normalParagraph("Понятие вычислительной стойкости предполагает, что алгоритм является стойким, если для его взлома требуются вычислительные ресурсы, превышающие практически доступные. В отличие от теоретико-информационной стойкости, где секретность гарантируется математически (как в случае одноразового блокнота), вычислительная стойкость основана на сложности решения определенных математических задач."));

  children.push(normalParagraph("Стандартизация криптографических алгоритмов осуществляется рядом организаций. Национальный институт стандартов и технологий (NIST, США) определяет стандарты AES, DES, 3DES и публикует рекомендации по оценке случайности (SP 800-22). В Российской Федерации действуют стандарты ГОСТ 28147-89 и ГОСТ Р 34.12-2018 (алгоритмы \u00ABМагма\u00BB и \u00ABКузнечик\u00BB)."));
  ...
  children.push(normalParagraph("AES (Advanced Encryption Standard) \u2013 симметричный блочный шифр, принятый в качестве стандарта шифрования правительством США в 2001 году по результатам открытого конкурса NIST. Победителем конкурса стал алгоритм Rijndael, разработанный бельгийскими криптографами Й. Дайменом и В. Рэйменом."));
  ...
  children.push(normalParagraph("Blowfish \u2013 симметричный блочный шифр, разработанный Б. Шнайером в 1993 году как свободная альтернатива существующим алгоритмам. Алгоритм основан на сети Фейстеля, выполняет 16 раундов, оперирует блоками размером 64 бита и поддерживает переменную длину ключа от 32 до 448 бит."));
  ...
  children.push(normalParagraph("RC4 (Rivest Cipher 4) \u2013 потоковый шифр, разработанный Р. Ривестом в 1987 году для компании RSA Security. Алгоритм генерирует псевдослучайный ключевой поток на основе перестановки массива из 256 байт (S-блока), инициализированного секретным ключом."));

  children.push(normalParagraph("RC4 отличается простотой реализации и высокой скоростью работы, что обеспечило его широкое распространение в протоколах SSL/TLS и WEP/WPA. Однако были обнаружены серьезные уязвимости: статистическое смещение (bias) в начальных байтах ключевого потока, уязвимости в протоколе WEP, а также атаки на RC4 в контексте TLS. В 2015 году IETF запретил использование RC4 в TLS (RFC 7465)."));
  ...
  children.push(normalParagraph("ChaCha20 \u2013 современный потоковый шифр, разработанный Д. Бернстайном в 2008 году как модификация алгоритма Salsa20. ChaCha20 использует 256-битный ключ, 96-битный одноразовый вектор инициализации (nonce) и 32-битный счетчик блоков."));
  ...
  children.push(normalParagraph("Таблица 1.1 \u2013 Характеристики исследуемых алгоритмов шифрования", {
    alignment: AlignmentType.CENTER, noIndent: true, fontSize: FONT_SIZE_SMALL, spacingAfter: 60,
  }));
...
function generateChapter2() {
  ...
  children.push(normalParagraph("Связь между теорией информации и криптографией была установлена К. Шенноном в фундаментальной работе \u00ABCommunication Theory of Secrecy Systems\u00BB (1949). Шеннон показал, что задача шифрования может быть формализована в терминах теории информации, а криптостойкость алгоритма может быть оценена через информационно-теоретические характеристики шифртекста."));

  children.push(normalParagraph([
    { text: "Центральным понятием теории информации является " },
    { text: "энтропия", bold: true },
    { text: " \u2013 мера неопределенности (количества информации) случайной величины. Шеннон определил энтропию дискретной случайной величины X, принимающей значения x\u2081, x\u2082, ..., x\u2099 с вероятностями p(x\u2081), p(x\u2082), ..., p(x\u2099), формулой:" },
  ]));
  ...
  children.push(normalParagraph("Энтропия Шеннона является основной метрикой оценки случайности шифртекста. Для последовательности байтов C = {c\u2081, c\u2082, ..., c\u2099} энтропия вычисляется как:"));
  ...
  children.push(normalParagraph("KL-дивергенция количественно оценивает отличие эмпирического распределения байтов шифртекста от идеального равномерного распределения. Для распределения P шифртекста и равномерного распределения Q = U(0, 255) KL-дивергенция определяется как:"));
  ...
  children.push(normalParagraph("Энтропийные метрики дополняются набором статистических тестов, оценивающих различные аспекты распределения байтов шифртекста."));
  ...
  children.push(normalParagraph("Лавинный эффект является одним из ключевых свойств стойкого шифра, обеспечивающим диффузию \u2013 равномерное распространение влияния каждого бита открытого текста на все биты шифртекста. Строгий лавинный критерий (SAC), предложенный Вебстером и Тавалесом (1986), требует, чтобы при инвертировании любого одного бита входных данных каждый бит выходных данных изменялся с вероятностью 1/2."));
  ...
  children.push(normalParagraph("Каждая из рассмотренных метрик оценивает определенный аспект криптостойкости, однако по отдельности они не дают полной картины. Энтропия Шеннона характеризует равномерность распределения, но не учитывает зависимость от открытого текста. KL-дивергенция оценивает отклонение от равномерности, но не измеряет диффузию. Лавинный эффект характеризует диффузию, но не информационную утечку. Поэтому для всесторонней оценки необходим комплексный подход."));
...
function generateChapter3() {
  ...
  children.push(normalParagraph([
    ...
    { text: "\u2014 современный веб-фреймворк для построения REST API. Обеспечивает автоматическую генерацию документации OpenAPI (Swagger), валидацию входных данных через Pydantic, асинхронную обработку запросов и высокую производительность благодаря использованию ASGI-сервера Uvicorn." },
  ]));

  children.push(normalParagraph([
    { text: "PyCryptodome ", bold: true },
    { text: "\u2014 криптографическая библиотека, предоставляющая реализации стандартных алгоритмов шифрования (AES, DES, 3DES, Blowfish, RC4). Поддерживает различные режимы шифрования (ECB, CBC, CTR) и является FIPS-совместимой. Алгоритм ГОСТ 28147-89 реализован вручную на основе спецификации стандарта." },
  ]));

  children.push(normalParagraph([
    { text: "NumPy, SciPy, Pandas ", bold: true },
    { text: "\u2014 библиотеки научных вычислений. NumPy используется для побайтового анализа данных и вычисления частотных распределений. SciPy \u2014 для статистических тестов (критерий хи-квадрат). Pandas \u2014 для агрегации результатов экспериментов и ранжирования алгоритмов." },
  ]));

  children.push(normalParagraph([
    { text: "React 19 ", bold: true },
    { text: "\u2014 JavaScript-библиотека для построения пользовательских интерфейсов. Компонентный подход обеспечивает переиспользование элементов интерфейса, а реактивная модель данных \u2014 автоматическое обновление визуализации при получении результатов." },
  ]));

  children.push(normalParagraph([
    { text: "Recharts ", bold: true },
    { text: "\u2014 библиотека визуализации данных на основе React и D3.js. Используется для построения столбчатых диаграмм, линейных графиков и других визуализаций результатов экспериментов." },
  ]));
...
function generateChapter4() {
  ...
  children.push(normalParagraph("В таблице представлены средние значения энтропии Шеннона шифртекста для каждого алгоритма, усредненные по всем типам данных и размерам."));
  ...
  children.push(normalParagraph("Расхождение Кульбака-Лейблера характеризует отклонение распределения байтов шифртекста от равномерного. Средние значения KL-дивергенции приведены в таблице."));
  ...
  children.push(normalParagraph("Тест лавинного эффекта оценивает диффузионные свойства шифра: при изменении одного бита открытого текста должно изменяться приблизительно 50% бит шифртекста (идеальный лавинный коэффициент AC = 0.5)."));
  ...
  children.push(normalParagraph("Критерий хи-квадрат проверяет гипотезу о равномерности распределения байтов шифртекста. Результаты приведены в таблице."));
  ...
  children.push(normalParagraph([
    ...
    { text: "что может показаться неожиданным, учитывая малый размер ключа (56 бит). Однако комплексный показатель оценивает статистические свойства шифртекста, а не криптографическую стойкость к атаке полным перебором. По энтропийным и корреляционным характеристикам DES демонстрирует качественную работу." },
  ]));
  ...
  children.push(normalParagraph([
    ...
    { text: "преимущественно из-за практического отсутствия лавинного эффекта (S_avalanche = 0.0001). Это фундаментальное свойство потоковых шифров, а не конкретная уязвимость RC4. По остальным метрикам (энтропия, корреляция) RC4 показывает результаты, сопоставимые с блочными шифрами." },
  ]));

  children.push(normalParagraph([
    { text: "5. ГОСТ 28147-89 сопоставим с западными стандартами ", bold: true },
    { text: "по всем метрикам, кроме KL-дивергенции, где занимает последнее место среди блочных шифров. Абсолютные различия крайне малы." },
  ]));
  ...
  children.push(listItem("Для обеспечения максимальной криптографической стойкости рекомендуется использовать AES-256 как текущий стандарт с наибольшим размером ключа и подтвержденным качеством шифрования.", "numbered"));
  children.push(listItem("DES и 3DES не рекомендуются для новых систем из-за малого размера ключа, несмотря на хорошие статистические показатели шифртекста.", "numbered"));
  ...
  children.push(normalParagraph("Проведены экспериментальные исследования шести алгоритмов шифрования (AES-256, DES, 3DES, Blowfish, RC4, ГОСТ 28147-89) с использованием разработанной платформы CryptoAnalyzer. Выполнено 126 экспериментов энтропийного и статистического анализа и 18 серий тестов лавинного эффекта."));
  ...
  children.push(normalParagraph("Выявлено фундаментальное различие между блочными и потоковыми шифрами по критерию лавинного эффекта: блочные шифры обеспечивают AC \u2248 0.24 (с учётом режима CBC), потоковый RC4 \u2014 AC \u2248 0.0001."));
  ...
  children.push(normalParagraph("Направления развития: добавление асимметричных алгоритмов (RSA, ECC), интеграция тестов NIST SP 800-22, анализ производительности (throughput), поддержка дополнительных режимов шифрования (CTR, GCM) и возможность загрузки пользовательских алгоритмов."));
...
function generateIntroduction() {
  ...
  children.push(normalParagraph([
    ...
    { text: "В условиях стремительного развития информационных технологий и повсеместной цифровизации общества защита информации становится одной из ключевых задач. Алгоритмы шифрования, являющиеся основой криптографической защиты, требуют всесторонней оценки их стойкости для обеспечения надежности информационных систем. Традиционные методы оценки криптостойкости, основанные на теоретическом криптоанализе, не всегда доступны практикам и не позволяют проводить быстрое сравнение алгоритмов. Энтропийный анализ и статистические методы оценки предоставляют мощный инструментарий для количественной оценки качества шифрования, однако существующие программные средства не обеспечивают комплексного подхода, объединяющего различные метрики в единую систему оценки." },
  ]));
  ...
  children.push(normalParagraph([
    ...
    { text: "теория информации (энтропия Шеннона, расхождение Кульбака-Лейблера), математическая статистика (критерий хи-квадрат, корреляционный анализ), методы программной инженерии (объектно-ориентированное проектирование, REST-архитектура, контейнеризация)." },
  ]));

  children.push(normalParagraph([
    { text: "Научная новизна ", bold: true },
    { text: "заключается в разработке комплексной методики оценки криптостойкости, объединяющей четыре энтропийные метрики, статистические тесты и тест лавинного эффекта в агрегированный показатель S_total, позволяющий количественно ранжировать алгоритмы шифрования по единой шкале." },
  ]));
...
function generateConclusion() {
  ...
  children.push(normalParagraph("1. Проведен анализ шести алгоритмов шифрования (AES-256, DES, 3DES, Blowfish, RC4, ГОСТ 28147-89) и систематизированы существующие подходы к оценке криптостойкости: формальные методы, статистические тесты NIST, энтропийный анализ и тест лавинного эффекта."));

  children.push(normalParagraph("2. Систематизированы четыре энтропийные метрики (энтропия Шеннона, расхождение Кульбака-Лейблера, условная энтропия, взаимная информация) и статистические тесты (критерий хи-квадрат, корреляционный анализ, метрики распределения). Для каждой метрики определены формулы, пороговые значения и критерии интерпретации."));

  children.push(normalParagraph("3. Разработана комплексная методика оценки криптостойкости, включающая шесть этапов: генерация тестовых данных семи типов, шифрование, расчет энтропийных метрик, расчет статистических метрик, тест лавинного эффекта и агрегация результатов. Предложен комплексный показатель S_total, вычисляемый как среднее четырех нормализованных частных баллов."));
...
function generateBibliography() {
  ...
  const sources = [
    "Шеннон К.Э. Теория связи в секретных системах // Работы по теории информации и кибернетике. \u2014 М.: ИЛ, 1963. \u2014 С. 333\u2013402.",
    ...
    "Столлингс В. Криптография и защита сетей: принципы и практика. \u2014 М.: Вильямс, 2001. \u2014 672 с.",
    "ГОСТ 28147-89. Системы обработки информации. Защита криптографическая. Алгоритм криптографического преобразования. \u2014 М.: Стандартинформ, 1989.",
    "FIPS 197. Advanced Encryption Standard (AES). \u2014 NIST, 2001.",
    "FIPS 46-3. Data Encryption Standard (DES). \u2014 NIST, 1999.",
    "NIST SP 800-67. Recommendation for the Triple Data Encryption Algorithm (TDEA) Block Cipher. \u2014 NIST, 2017.",
    "Schneier B. Description of a New Variable-Length Key, 64-Bit Block Cipher (Blowfish) // Fast Software Encryption. \u2014 Springer, 1994. \u2014 P. 191\u2013204.",
    "Rivest R.L. The RC4 Encryption Algorithm. \u2014 RSA Data Security, Inc., 1992.",
    "Bernstein D.J. ChaCha, a variant of Salsa20 // Workshop Record of SASC 2008. \u2014 2008.",
    "NIST SP 800-22 Rev. 1a. A Statistical Test Suite for Random and Pseudorandom Number Generators for Cryptographic Applications. \u2014 NIST, 2010.",
    "Kullback S., Leibler R.A. On Information and Sufficiency // The Annals of Mathematical Statistics. \u2014 1951. \u2014 Vol. 22, No. 1. \u2014 P. 79\u201386.",
    "Webster A.F., Tavares S.E. On the Design of S-Boxes // Advances in Cryptology \u2014 CRYPTO\u201985. \u2014 Springer, 1986. \u2014 P. 523\u2013534.",
    "Feistel H. Cryptography and Computer Privacy // Scientific American. \u2014 1973. \u2014 Vol. 228, No. 5. \u2014 P. 15\u201323.",
    "Daemen J., Rijmen V. The Design of Rijndael: AES \u2014 The Advanced Encryption Standard. \u2014 Springer, 2002. \u2014 238 p.",
    "Biham E., Shamir A. Differential Cryptanalysis of DES-like Cryptosystems // Journal of Cryptology. \u2014 1991. \u2014 Vol. 4, No. 1. \u2014 P. 3\u201372.",
    "Matsui M. Linear Cryptanalysis Method for DES Cipher // Advances in Cryptology \u2014 EUROCRYPT\u201993. \u2014 Springer, 1994. \u2014 P. 386\u2013397.",
    "Fluhrer S., Mantin I., Shamir A. Weaknesses in the Key Scheduling Algorithm of RC4 // Selected Areas in Cryptography \u2014 SAC 2001. \u2014 Springer, 2001. \u2014 P. 1\u201324.",
    "Katz J., Lindell Y. Introduction to Modern Cryptography. \u2014 CRC Press, 2020. \u2014 3rd ed. \u2014 626 p.",
    "Paar C., Pelzl J. Understanding Cryptography: A Textbook for Students and Practitioners. \u2014 Springer, 2010. \u2014 372 p.",
    "Cover T.M., Thomas J.A. Elements of Information Theory. \u2014 Wiley, 2006. \u2014 2nd ed. \u2014 776 p.",
    "Soto J. Statistical Testing of Random Number Generators // NIST, 1999.",
    "Rukhin A. et al. A Statistical Test Suite for Random and Pseudorandom Number Generators for Cryptographic Applications // NIST SP 800-22. \u2014 2010.",
    "Hamming R.W. Error Detecting and Error Correcting Codes // Bell System Technical Journal. \u2014 1950. \u2014 Vol. 29, No. 2. \u2014 P. 147\u2013160.",
    "FastAPI Documentation. \u2014 URL: https://fastapi.tiangolo.com/ (дата обращения: 15.03.2026).",
    "React Documentation. \u2014 URL: https://react.dev/ (дата обращения: 15.03.2026).",
    "PyCryptodome Documentation. \u2014 URL: https://pycryptodome.readthedocs.io/ (дата обращения: 15.03.2026).",
    "Docker Documentation. \u2014 URL: https://docs.docker.com/ (дата обращения: 15.03.2026).",
    "NumPy Documentation. \u2014 URL: https://numpy.org/doc/ (дата обращения: 15.03.2026).",
    "SciPy Documentation. \u2014 URL: https://docs.scipy.org/ (дата обращения: 15.03.2026).",
    "Recharts Documentation. \u2014 URL: https://recharts.org/ (дата обращения: 15.03.2026).",
    "Алфёров А.П., Зубов А.Ю., Кузьмин А.С., Черёмушкин А.В. Основы криптографии. \u2014 М.: Гелиос АРВ, 2001. \u2014 480 с.",
