...
def generate_markdown_report(
    ...
    conclusions = (
        ...
        f"4. Блочные шифры (AES, DES, Blowfish) демонстрируют лучший лавинный эффект "
        f"благодаря многораундовым подстановкам. Потоковые шифры (RC4, ChaCha20) "
        f"выполняют XOR с ключевым потоком, что не создаёт лавинного эффекта при "
        f"изменении открытого текста.\n\n"
        f"5. DES и RC4 не рекомендуются к использованию в новых системах. "
        f"Для задач, требующих высокой криптостойкости, следует применять AES-256 или ChaCha20."
    )
